...
class App extends Component {
  ...
  makeBet = (prediction, amount, betOn) => {
    ...
    this.state.mBetContractV3Instance.methods.createBet(betOn, kovanTokenConversionAddress, prediction).send({ from : this.state.mAccount, value: amount })
  ...
  makeCounterBet = (bettingPairId, prediction, amount, betOn) => {
    this.setState({ mLoading: true })
    let kovanTokenConversionAddress = this.state.mKovanTokenConversions.get(betOn)

    // joinBet(uint256 interestedBet, int pricePrediction)
    this.state.mBetContractV3Instance.methods.joinBet(bettingPairId, prediction).send({ from: this.state.mAccount, value: amount })
    .on('transactionHash', (hash) => {
